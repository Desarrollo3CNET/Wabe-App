...
const SuspencionDelanteraScreen = ({ navigation }) => {
  const isTablet = useSelector((state) => state.app.isTablet);
  const dispatch = useDispatch();
  const articulos = useSelector(
    (state) => state.revision.articulosMantenimiento[5],
  );
  ...
  const handleNext = async () => {
    ...
    if (isValid) {
      navigation.navigate('FrenosTraserosScreen');
    } else {
    ...
    <View style={styles.container}>
      ...
      <View style={styles.content}>
        <ScrollView contentContainerStyle={styles.scrollContent}>
          <Text style={styles.sectionTitle}>Rodamientos Traseros</Text>
      ...
      <FooterButtonsRevision
        onBack={() => navigation.navigate('SuspencionTraseraScreen')}
...
export default SuspencionDelanteraScreen;
