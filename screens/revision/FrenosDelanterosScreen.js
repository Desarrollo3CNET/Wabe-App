...
const FrenosDelanterosScreen = ({ navigation }) => {
  const dispatch = useDispatch();

  const articulos = useSelector(
    (state) => state.revision.articulosMantenimiento[3],
  );
  ...
  const handleNext = async () => {
    ...
    if (isValid) {
      navigation.navigate('SuspencionTraseraScreen');
    } else {
    ...
    <View style={styles.container}>
      ...
      <View style={styles.content}>
        <ScrollView contentContainerStyle={styles.scrollContent}>
          <Text style={styles.sectionTitle}>Frenos Delanteros</Text>
      ...
      <FooterButtonsRevision
        onBack={() => navigation.navigate('RodamientosDelanterosScreen')}
...
export default FrenosDelanterosScreen;
