...
const SuspencionTraseraScreen = ({ navigation }) => {
  const isTablet = useSelector((state) => state.app.isTablet);
  const dispatch = useDispatch();
  const articulos = useSelector(
    (state) => state.revision.articulosMantenimiento[4],
  );
  ...
  const handleNext = async () => {
    ...
    if (isValid) {
      navigation.navigate('RodamientosTraserosScreen');
    } else {
    ...
    <View style={styles.container}>
      ...
      <View style={styles.content}>
        <ScrollView contentContainerStyle={styles.scrollContent}>
          <Text style={styles.sectionTitle}>Suspensión Trasera</Text>
      ...
      <FooterButtonsRevision
        onBack={() => navigation.navigate('FrenosDelanterosScreen')}
...
export default SuspencionTraseraScreen;
